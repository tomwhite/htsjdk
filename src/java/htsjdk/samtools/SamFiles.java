...
public class SamFiles {
    ...
    public static File findIndex(final File samFile) {
        ...
        if (fileName.endsWith(BamFileIoUtils.BAM_FILE_EXTENSION)) {
            ...
            final String crai = fileName.substring(0, fileName.length() - CramIO.CRAM_FILE_EXTENSION.length()) + CRAIIndex.CRAIIndexSuffix;
            indexFile = new File(samFile.getParent(), crai);
            if (indexFile.isFile())
                return indexFile;

            indexFile = new File(samFile.getParent(), samFile.getName() + CRAIIndex.CRAIIndexSuffix);
            if (indexFile.isFile())
                return indexFile;
        }
